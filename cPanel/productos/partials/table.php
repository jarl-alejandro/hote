...
<table class=" highlight responsive-table bordered z-depth-1 centered" id="Tab_Filter">
 <thead class="red darken-4 white-text">
   <tr>
     ...
     <th data-field="nombre">Nombre del Producto/Servicio</th>
 ...
 <tbody class="white">
   <?php
   include '../../../bd/db.php';
   $productos = $pdo->query("SELECT * FROM hotel_producto");
   ...
   <tr>
     ...
     <td>
       <button class="btn waves-effect waves-light blue darken-4 editar" data-codigo="<?= $row["codigo_producto"] ?>"><i class="material-icons">edit</i></button>
       <button class="btn waves-effect waves-light red darken-4 eliminar" data-codigo="<?= $row["codigo_producto"] ?>"><i class="material-icons">delete</i></button>
...
<script>
  $.getScript("productos/static/js/actions.js")
