...
$header = array(utf8_decode('Nº'), 'CLIENTE', 'FECHA', 'PRECIO', 'DEPOSITO', "TIPO");
...
$fecha = date("Y/m/d");
$query = $pdo->query("SELECT * FROM v_pagos_mensuales");
...
foreach ($facturas as $row) {
  ...
  $pdf->Cell(30, 6.5, $id, 1, 0, 'C');
  $pdf->Cell(100, 6.5, $row['cliente'], 1, 0, 'C');
  $pdf->Cell(50, 6.5, $row['fecha_facturam'], 1, 0, 'C');
  $pdf->Cell(30, 6.5, $row['total_facturam'], 1, 0, 'C');
  $pdf->Cell(50, 6.5, $row['factura_deposito'], 1, 0, 'C');
  $pdf->Cell(30, 6.5, $tipo, 1, 0, 'C');
