;(function (){
	...
	$("#reporteGeneral").on("click", function () {
		window.open (`./compras-fecha/reporte/lista.php`, "_blank","toolbar=yes, scrollbars=yes, resizable=yes, top=50, left=60, width=1200, height=600")
	})
	...
	$("#Aceptar").on("click", function (e) {
		...
		if(validarFechasByPagos()){
			if(type === "fecha") {
				window.open(`./compras-fecha/reporte/fecha.php?desde=${$desde.val()}&hasta=${$hasta.val()}`, "_blank","toolbar=yes, scrollbars=yes, resizable=yes, top=50, left=60, width=1200, height=600")
				document.getElementById('Aceptar').dataset.type = ""
			}
			else {
				$(".table").load(`compras-fecha/partials/fecha.php?desde=${$desde.val()}&hasta=${$hasta.val()}`)
			}
			close()
		}
	})
