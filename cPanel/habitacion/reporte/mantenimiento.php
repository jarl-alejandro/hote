...
class PDF extends FPDF {
  ...
  function Header() {
    ...
    $this->Text(110, 54, 'LISTADO DE HABITACIONES MANTENIMIENTO');
    $this->Ln(50);
...
$pdf -> SetY(65);
$header = array('CODIGO', 'NOMBRE', 'OBSERVACION', 'HABITACION');

$pdf -> SetX(5);
$pdf->SetFont('Arial', '',12);
$pdf->TablaColores($header);

$query = $pdo->query("SELECT * FROM hotel_habitacion WHERE estado_habitacion=5");
...
while ($row = $query->fetch()) {
  ...
  $pdf->Cell(100, 17, $row['nombre_habitacion'], 1, 'C');
  $pdf->Cell(100, 17, $row['obs_manten'], 1, 'C');
