<?php if($row["estado_habitacion"] == 0){ ?>
  <!-- Libre -->
  <article class="numeroHabitacion card col s1 over-hab"
...
<?php } else if($row["estado_habitacion"] == 10){ ?>
  ...
  class="numberHabi card col s1 over-hab" style="margin-right:1em !important;border:4px solid #B71C1C"
    data-id='<?= $row["codigo_habitacion"] ?>'>
    <h4><?= $row["nombre_habitacion"] ?></h4>
  </article>
<?php } else if($row["estado_habitacion"] == 1){
  ...
  if($tmp->rowCount() > 0){  ?>
    ...
    class="numberHabi card col s1 over-hab" style="margin-right:1em !important;border:4px solid #bfa145"
      data-id='<?= $row["codigo_habitacion"] ?>'>
      <h4><?= $row["nombre_habitacion"] ?></h4>
    </article>

  <?php } else { ?>
    <article class="numeroHabitacion card col s1 over-hab" style="margin-right:1em !important"
