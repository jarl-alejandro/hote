 ...
 <thead class="red darken-4 white-text">
  ...
  date_default_timezone_set('America/Guayaquil');
   ...
   $factura = $pdo->query("SELECT * FROM hotel_facturam WHERE factura_estado='pagado' 
      AND fecha_facturam='$fecha_actual'");
   $restaurante = $pdo->query("SELECT * FROM hotel_restaurante WHERE fecha_restaurante='$fecha_actual'");
...
<script>
  $.getScript("cierre_caja/static/js/actions.js")
</script>
