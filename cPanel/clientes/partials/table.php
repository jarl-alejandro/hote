...
<table class=" highlight responsive-table bordered z-depth-1 centered" id="Tab_Filter">
 ...
 <tbody class="white">
   <?php
   include '../../../bd/db.php';
   $clientes = $pdo->query("SELECT * FROM hotel_cliente WHERE cedula_cliente != 'xxxxxxxxxx'");
   ...
   <tr>
     ...
     <td>
       <button class="btn waves-effect waves-light blue darken-4 editar" data-cedula="<?= $row["cedula_cliente"] ?>"><i class="material-icons">edit</i></button>

       <?php if ($row["estado_cliente"] == 1){ ?>
       <button disabled class="btn waves-effect waves-light color-toolbar ingresar" data-cedula="<?= $row['cedula_cliente'] ?>"><i class="material-icons">trending_flat</i></button>
       ...
       <?php } else if ($row["estado_cliente"] == 2) { ?>
        <button class="btn waves-effect waves-light color-toolbar desbloquear" data-cedula="<?= $row["cedula_cliente"] ?>"><i class="material-icons">https</i></button>
       <?php } ?>
       <button class="btn waves-effect waves-light red darken-4 eliminar" data-cedula="<?= $row["cedula_cliente"] ?>"><i class="material-icons">delete</i></button>
...
<script>
  $.getScript("clientes/static/js/actions.js")
